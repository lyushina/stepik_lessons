...
class ProductPage(BasePage):
    ...
    def should_be_after_product_page(self):
        self.should_be_product_title_in_basket()
        self.should_be_cart_amount_in_basket()
    ...
    def should_not_be_disappeared_message_after_adding_product_to_basket(self):
    ...
    def check_product_and_basket_price_same(self):
        assert self.get_cart_amount() == self.get_product_price(), f'Basket price and product price are not the same' \
                                                                   f'\n {self.browser.current_url}'

    def check_product_and_alert_same(self):
        assert self.get_product_title() == self.get_product_title_in_basket(), \
